...
| password_hash | string   | NOT NULL         | Пароль (хэш)      |
| created_at    | datetime |                  | Дата регистрации  |
| role_id       | int      | FK → ROLE(id)    | Роль пользователя |
...
| user_id    | int    | FK, UNIQUE → USER(id) | Ссылка на пользователя           |
...
| user_id         | int      | FK → USER(id)         | Автор объявления       |
| modification_id | int      | FK → MODIFICATION(id) | Модификация автомобиля |
...
| listing_id | int    | FK → LISTING.id | Объявление                |
...
| Поле       | Тип | Ограничения          | Описание                             |
| ---------- | --- | -------------------- | ------------------------------------ |
| user_id    | int | PK, FK → USER(id)    | Пользователь, добавивший в избранное |
| listing_id | int | PK, FK → LISTING(id) | Объявление, которое в избранном      |
...
| country_id | int    | FK → COUNTRY.id | Страна                      |
...
| manufacturer_id | int    | FK → MANUFACTURER.id | Производитель        |
...
| start_year   | int    |                   | Год начала выпуска      |
| car_model_id | int    | FK → CAR_MODEL.id | Модель                  |
...
| generation_id   | int    | FK → GENERATION.id   | Поколение                 |
| engine_id       | int    | FK → ENGINE.id       | Двигатель                 |
| transmission_id | int    | FK → TRANSMISSION.id | Трансмиссия               |
| body_id         | int    | FK → BODY.id         | Кузов                     |
...
| power_kw      | int     | CHECK (> 0) | Мощность (кВт)           |
| torque_nm     | int     | CHECK (> 0) | Крутящий момент (Н·м)    |
...
| gears_num | int    | CHECK (> 0) | Количество передач        |
...
| user_id   | int      | FK → USER.id | Пользователь         |
